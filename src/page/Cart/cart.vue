                ...
                <div class="cart-group divide pr" :data-productid="item.productId">
                  <div class="cart-top-items">
                    <div class="cart-items clearfix">
                      ...
                      <span class="blue-checkbox-new " :class="{'checkbox-on':item.productSelected === true}"
                      ...
                      <div>
                        ...
                        <div class="price1">
                            <el-input-number 
                                ...
                                @change="EditNum($event,item.productId,item.productSelected)" 
                                :min="1" 
              ...
              <div class="cart-bar-operation">
                <div>
                  <div class="choose-all">
                    <span :class="{'checkbox-on':checkAllFlag}" class="blue-checkbox-new" @click="_editCheckAll"></span>
                    <span @click="_editCheckAll">全选</span>
                  </div>
  ...
  export default {
    ...
    computed: {
      ...
      checkedCount () {
        ...
        this.cartList && this.cartList.forEach((item) => {
          if (item.productSelected === true) i++
        })
        return Number(i)
      },
      // 计算总数量
    ...
    methods: {
      ...
      async _editCheckAll () {
        ...
        this.EDIT_CART({selected: checkAll})
        // editCheckAll({checked: checkAll}).then(res => {
        //   this.EDIT_CART({checked: checkAll})
        // })
      },
      // 修改购物车
      _cartEdit (productId, quantity, selected) {
        cartEdit(
          {
            productId,
            quantity,
      ...
      editCart (type, item) {
        if (type && item) {
          let checked = item.productSelected
          let productId = item.productId
          let quantity = item.quantity
          // 勾选
          if (type === 'check') {
            let newChecked = !checked
            this._cartEdit(productId, quantity, newChecked)
          }
        } else {
          console.log('缺少所需参数')
        }
      },
      EditNum (quantity, productId, selected) { // 数量
        this._cartEdit(productId,quantity,selected)
      },
      // 删除整条购物车
      cartdel (productId) {
