            ...
            <div class="gray-sub-title cart-title">
              <div class="first">
                <div>
                  ...
                  <span class="order-id"> 订单号： <a @click="orderDetail(item.orderNo)">{{item.orderNo}}</a> </span>
                </div>
                <div class="f-bc">
              ...
              <div class="last">
                ...
                <span class="order-detail"> <a @click="orderDetail(item.orderNo)">查看详情 ><em class="icon-font"></em></a> </span>
              </div>
            </div>
            <div class="pr">
              <div class="cart" v-for="(good,j) in item.orderItemVoList" :key="j">
                <div class="cart-l" :class="{bt:j>0}">
                  <div class="car-l-l">
                    <div class="img-box"><a @click="goodsDetails(good.productId)"><img :src="good.productImage" alt=""></a></div>
                    <div class="ellipsis"><a style="color: #626262;" @click="goodsDetails(good.productId)">{{good.productName}}</a></div>
                  </div>
                  <div class="cart-l-r">
                    <div>¥ {{Number(good.currentUnitPrice).toFixed(2)}}</div>
                    <div class="num">{{good.quantity}}</div>
                    <div class="type">
                      <el-button style="margin-left:20px" @click="_delOrder(item.orderNo,i)" type="danger" size="small" v-if="j<1" class="del-order">删除此订单</el-button>
              ...
              <div class="prod-operation pa" style="right: 0;top: 0;">
                ...
                <div v-if="item.status === 10">
                  <el-button @click="orderPayment(item.orderNo)" type="primary" size="small">现在付款</el-button>
                </div>
                <div class="status" v-if="item.status !== 10"> {{getOrderStatus(item.status)}}  </div>
              </div>
            </div>
          </div>
        </div>
